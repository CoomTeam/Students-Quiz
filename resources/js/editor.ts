...
async function loadQuestion() {
	...
	// Get question
	const question: Question = await POST('/admin/quiz-editor/getQuestion', {'id': CHOSEN_QUESTION});
 ...
 */
 async function loadAnswer() {
	...
	const answer: Answer = await POST('/admin/quiz-editor/getAnswer', {'id': CHOSEN_ANSWER});
...
async function newAnswer() {
	...
	const resp = await POST('/admin/quiz-editor/newAnswer', {'id': CHOSEN_QUESTION});
...
async function newQuestion() {
	...
	const resp = await POST('/admin/quiz-editor/newQuestion');
...
async function deleteQuestion() {
	if (NEED_SAVE) return pleaseSave();
	before_question();
	await POST('/admin/quiz-editor/deleteQuestion', {id: CHOSEN_QUESTION});
...
async function deleteAnswer() {
	if (NEED_SAVE) return pleaseSave();
	before_question();
	await POST('/admin/quiz-editor/deleteAnswer', {id: CHOSEN_ANSWER});
	chooseAnswer(-1);
	await loadQuestion();
	after_question();
}
...
async function save() {
	...
	await POST('/admin/quiz-editor/save', req);
...
async function chooseQuestion(id: number) {
	...
	QUESTION_LIST.update(
		await POST('/admin/quiz-editor/getAllQuestions')
	);
}
